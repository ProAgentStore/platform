...
keysRoutes.get('/providers', async (c) => {
  return c.json({ providers: PROVIDERS.map(p => ({ id: p.id, name: p.name, docsUrl: p.docsUrl })) });
});
...
keysRoutes.get('/status', async (c) => {
  ...
  ).bind(session.uid).all<{ provider: string; created_at: string; last_used_at: string | null }>();
  ...
  return c.json({
    providers: PROVIDERS.map(p => ({
      ...
      lastUsedAt: results.find(r => r.provider === p.id)?.last_used_at || null,
    })),
  });
});
...
keysRoutes.put('/:provider', async (c) => {
  ...
  if (provider.keyPrefix && !key.startsWith(provider.keyPrefix)) {
    throw new HttpError(400, `${provider.name} keys should start with "${provider.keyPrefix}"`);
  }

  const { ciphertext, dekWrapped, iv } = await encryptKey(key, c.env.KEY_ENCRYPTION_KEY);
  ...
  ).bind(session.uid, providerId, ciphertext, dekWrapped, iv).run();
...
keysRoutes.delete('/:provider', async (c) => {
  ...
  ).bind(session.uid, providerId).run();
...
keysRoutes.all('/proxy/:host{.+}', async (c) => {
  ...
  ).bind(session.uid, hour).first<{ count: number }>();
  ...
  ).bind(session.uid, hour).run();

  // Decrypt user's key
  const row = await c.env.DB.prepare(
    'SELECT key_ciphertext, dek_wrapped, iv FROM user_api_keys WHERE user_id = ?1 AND provider = ?2',
  ).bind(session.uid, providerId).first<{ key_ciphertext: ArrayBuffer; dek_wrapped: ArrayBuffer; iv: ArrayBuffer }>();
  if (!row) throw new HttpError(400, `No ${providerId} key stored. Add it in your profile.`);
