...
async function importKek(kekHex: string): Promise<CryptoKey> {
  const raw = new Uint8Array(kekHex.match(/.{2}/g)!.map(b => parseInt(b, 16)));
...
export async function encryptKey(plaintext: string, kekHex: string): Promise<{
...
export async function decryptKey(
  ...
  const dek = await crypto.subtle.unwrapKey(
    'raw', dekWrapped, kek, 'AES-KW',
    { name: 'AES-GCM', length: 256 }, false, ['decrypt'],
  );
  const plainBuf = await crypto.subtle.decrypt({ name: 'AES-GCM', iv }, dek, ciphertext);
