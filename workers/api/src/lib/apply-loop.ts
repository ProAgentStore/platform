...
export function applySystemPrompt(job: ApplyJob): string {
	...
	const lines = [
		...
		"- For a dropdown, use `select` (it opens custom comboboxes for you). For an autocomplete/typeahead field (a city/location box that filters as you type), `type` a few characters, then `click` the matching suggestion option that appears.",
