...
export function applySystemPrompt(job: ApplyJob): string {
	...
	const lines = [
		...
		"- VALIDATION FEEDBACK: after you type/select, the log reports the field's ACTUAL value and any validation error — e.g. `⚠ \"Mobile Phone\" REJECTED: \"Invalid phone number format\"` or `\"Mobile Phone\" now reads \"0404453580\"`. CRITICAL: if the log says the field NOW READS the value you sent and that value is plausibly valid, the value TOOK — a lingering \"REJECTED\"/format error is often STALE (shown mid-typing, clears on blur/submit). Do NOT keep retyping that field; move on and let the form re-validate at submit. Only treat it as a real rejection if the value did NOT take (reads empty or different) or the SAME field is still flagged AFTER a submit attempt. When a value truly didn't take (mask / country-code field), try a MATERIALLY DIFFERENT format, and after ~3 distinct tries call request_user_info — never resend a value that genuinely didn't take.",
		"- FILL EVERY REQUIRED FIELD before submitting. Scan the snapshot for empty required fields (marked * or showing 'required' / 'This is a required Field') — e.g. Preferred First Name, Address Line 1 — and fill them from the candidate data (or request_user_info). If a submit is blocked, it is usually an EMPTY required field, NOT the field you just filled — find and fill the empty one instead of re-editing a filled field.",
