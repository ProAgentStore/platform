...
export async function signSession(
  ...
  const data = b64url(new TextEncoder().encode(JSON.stringify(payload)).buffer as ArrayBuffer);
...
export async function verifySession(
  ...
  const payload: SessionPayload = JSON.parse(new TextDecoder().decode(unb64url(data)));
