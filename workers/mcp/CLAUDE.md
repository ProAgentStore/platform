...
**237 tool registrations** (`.tool(` in the files above): 21 in `index.ts`, 13 in
`coding-tools.ts` and 2 in `coding-engine-tools.ts` — all fifteen behind the `groups.has("coding")` gate — 14 in
`storage-tools.ts`, and 187 across `instance-tools/`. 211 are always registered; 26 are
...
+ 2 + 14 + 187 = 237. They said 88 until #602, which made the paragraph sum to 132 — a total the
