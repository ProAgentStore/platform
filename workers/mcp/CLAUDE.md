...
| Audit | `src/safety.ts` | `audit()` writes `audit:{subject}:{iso}:{uuid}` to `OAUTH_KV` with a 90-day TTL. No-ops without `OAUTH_KV` or a subject. A per-call `token` argument still HAS a subject: `safety()` attaches `resolveSubject`, which `audit()`/`listAuditEvents()` await, and `index.ts`'s `resolveTokenSubject` verifies the session HMAC (memoised, one entry, `exp`-honouring) to get its `uid` (#702). Before that, `token` set `subject: undefined` and a scripted caller wrote NO row from any tool, mutating or not. `listAuditEvents` over-lists to the KV cap and sorts the key NAMES before loading — KV lists lexicographically, so asking for `limit` keys returns the OLDEST (#704). Cap 200. |
...
**236 tool registrations** (`.tool(` in the files above): 21 in `index.ts`, 13 in
`coding-tools.ts` and 2 in `coding-engine-tools.ts` — all fifteen behind the `groups.has("coding")` gate — 14 in
`storage-tools.ts`, and 186 across `instance-tools/`. 210 are always registered; 26 are
...
+ 2 + 14 + 186 = 236. They said 88 until #602, which made the paragraph sum to 132 — a total the
...
`instance-tools/knowledge.ts`; and said 21 + 13 + 14 + 114 = 162 until #613's loop-presets group
added `get_instance_loop_presets` + `set_instance_loop_presets` to `instance-tools/composition.ts`; and said
21 + 13 + 14 + 116 = 164 until #613's product-feedback group added `record_instance_feedback` +
`delete_feedback` to `instance-tools/observability.ts`; and said 21 + 13 + 14 + 118 = 166 until #736
added `get_instance_connector_account` + `set_instance_connector_account` to
`instance-tools/connectors.ts`. The per-file rows in the tree above are
