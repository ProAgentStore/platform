...
const TABLE: Record<string, Row> = {
	...
	create_agent_version: ["agentAuthoring", "destructive", "create_agent_version", "envelope", "agent_id,confirm,description,dry_run,token"],
	...
	delete_agent: ["agentAuthoring", "destructive", "delete_agent", "envelope", "agent_id,confirm,dry_run,token"],
	...
	list_instance_connector_consents: ["connectors", "none", null, null, "instance_id,token"],
	...
	set_instance_connector_consent: ["connectors", "write", null, "envelope", "connector,dry_run,enabled,instance_id,mode,token"],
	...
	rollback_agent_version: ["agentAuthoring", "destructive", "rollback_agent_version", "envelope", "agent_id,confirm,dry_run,token,version_id"],
	...
	continue_instance_run: ["composition", "write", null, "envelope", "dry_run,instance_id,max_iterations,note,run_id,token"],
	...
	// #198: the read-only health probe. `none` like `whoami` — it must answer an unauthenticated
	// caller asking whether the platform is slow, so it has no scope gate and no auth refusal.
	platform_health: ["account", "none", null, null, "token"],
