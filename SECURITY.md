...
1. **End-user identity** — ProAgentStore's own Google + GitHub OAuth. The callback mints an **HS256 session JWT** (`lib/session.ts`) signed with `SESSION_SIGNING_KEY`; every API route verifies it locally (signature + `exp`, no `alg:none`) and **type-pins** it — `verifySession` rejects any token that carries a `typ` or lacks a `roles[]` array, so a WebSocket handshake token (`typ:"relay"`, `typ:"chat"`) or an OAuth `state` payload (all signed with the *same* key) can never be replayed as a full account session. We do **not** use Cloudflare Access for marketplace users. Return-to / redirect targets are allowlisted (`lib/origins.ts`); OAuth `state` is HMAC-signed and expiring.
2. **MCP** — OAuth 2.1 + PKCE (S256) with dynamic client registration. Tools carry scopes (`read` / `write` / `runtime` / `destructive`); destructive tools additionally require an explicit `confirm`. `MCP_READ_ONLY=1` blocks all mutating tools. Mutating tools gate **before dispatch** and write an audit record (`workers/mcp/src/safety.ts`). What the server *publishes* is held to the version that published it: `src/surface-lock.ts` hashes the served tool surface per version and `scripts/check-surface-lock.mjs --require-history` (a CI gate) reads the file's git history, so a recorded entry cannot be rewritten in place — the append-only rule is what stops a silent surface change shipping through a green build (#576).
...
3. **It would not close the exposure that is actually largest.** The OAuth handoff delivers the session **in a URL** (`?session=<JWT>`, `store/console/src/lib/auth.ts`). The console calls `history.replaceState` immediately, which handles the history entry but not `Referer` on subsequent cross-origin subresources, nor any intermediary that logs URLs. It survives a storage change untouched. (The chat WebSocket upgrade used to be the second half of this and **no longer is**: as of #317 it accepts only a short-lived, agent-scoped `typ:"chat"` token minted per connect at `POST /v1/agents/:id/ws-token`, and refuses the account JWT outright — the same shape the relay path adopted for `typ:"relay"`. Both are type-pinned against replay as a session. That was the higher-value follow-up, and it is done: getting a long-lived credential out of a query string was worth more than moving it between two script-reachable stores. The OAuth `?session=` handoff is what remains.)

**Compensating controls that are real today.**

- **Type-pinning.** `verifySession` rejects any token carrying a `typ` or lacking a `roles[]` array, so the two WebSocket handshake tokens (relay, chat) and the OAuth `state` payload — all signed with the *same* `SESSION_SIGNING_KEY` — cannot be replayed as an account session. It runs in both directions: `verifyChatToken` and `verifyRelayToken` likewise refuse an account session, which is what stops the old credential quietly continuing to work at either WS door.
...
- Real-time chat WebSockets authenticate the upgrade with a short-lived, agent-scoped `?token=` (minted at `POST /v1/agents/:id/ws-token`; the account JWT is refused) and pin the **server-verified** user id to the socket — a client cannot name another user's id.
...
- **Every mutating connector tool passes a per-instance write-consent gate** (`runRegistryTool` → `instance_connector_consent`). Consent is per instance, not per account, so subscribing to a second agent does not inherit the first one's authority.
- **"Does it change anything" and "may it be called" are separate fields, deliberately.** `mutates` answers the first; `scope` is what triggers the write-consent gate. Deriving one from the other produces a claim that is true on one axis and false on the other, so an audit reads `mutates` (`lib/tool-registry.ts`).
- **Reach is classified, not inferred, and fails closed.** Every tool declares the outermost boundary one call can cross — `platform`, `machine`, or `internet` (`lib/tool-reach.ts`). The tables are exhaustive and a tool reaching the listing without an entry fails the build; the fallback for an unclassified name is `internet`. This replaced a derived proxy ("does it name a connector") that told ten instances they had "no tool that reaches outside the platform" while every one of them held `fetch_url`, which POSTs caller-chosen bodies to caller-chosen hosts (#584).
- **Nothing may decide on a status value the application cannot write.** `lib/status-domain-guard.ts` parses the value domain each `*status` column declares in its own schema comment and holds the code that branches on those values to it, so a reader cannot wait forever on a state no writer produces (#570, #587, #590).
