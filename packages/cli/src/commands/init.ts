...
export const initCommand = new Command('init')
  ...
  .option('-t, --template <type>', `Template: ${TEMPLATES.join(', ')}`, 'worker')
  .action((name: string, opts: { template: string }) => {
    const slug = name.toLowerCase().replace(/[^a-z0-9-]/g, '-');
    const template = opts.template as TemplateType;

    if (!TEMPLATES.includes(template)) {
      console.error(`Unknown template: ${template}. Choose: ${TEMPLATES.join(', ')}`);
      process.exit(1);
    }
    ...
    const templateDir = resolve(__dirname, '..', '..', '..', '..', 'templates', `template-agent-${template}`);
    ...
    if (templateHasFiles) {
      ...
      writeFileSync(join(dir, 'agent.json'), JSON.stringify({
        ...
        name: name,
        description: '',
        storeType: template === 'api' ? 'tool' : template === 'cron' ? 'worker' : 'agent',
        category: 'general',
      ...
      if (template === 'worker') {
        wranglerLines.push('', '[[durable_objects.bindings]]', 'name = "AGENT"', 'class_name = "AgentDO"');
        wranglerLines.push('', '[[migrations]]', 'tag = "v1"', 'new_classes = ["AgentDO"]');
      }
      if (template === 'cron') {
        wranglerLines.push('', '[triggers]', 'crons = ["0 8 * * *"]');
      }
      writeFileSync(join(dir, 'wrangler.toml'), wranglerLines.join('\n') + '\n');

      // tsconfig
      writeFileSync(join(dir, 'tsconfig.json'), JSON.stringify({
        compilerOptions: {
          target: 'ESNext', module: 'ESNext', moduleResolution: 'bundler',
          lib: ['ESNext'], types: ['@cloudflare/workers-types'],
