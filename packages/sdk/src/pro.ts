...
export function initPro(config: ProAgentConfig): ProAgentStore {
  ...
  const ai: AiClient = {
    async run(model, input) {
      const result = await api<{ result: unknown }>(`/v1/agents/${agentId}/run`, {
        method: 'POST',
    ...
    async embed(model, text) {
      ...
      const result = await api<{ result: { data: number[][] } }>(`/v1/agents/${agentId}/run`, {
        method: 'POST',
        body: JSON.stringify({ input: { ...input, model } }),
      });
  ...
  const db: DbClient = {
    async query<T>(sql: string, params?: unknown[]): Promise<T[]> {
      ...
      throw new Error('Direct D1 queries not yet supported via SDK. Use the agent API.');
    },
  ...
  const storage: StorageClient = {
    async put(key, data) {
      ...
      await api(`/v1/agents/${agentId}/memory`, {
        method: 'PUT',
        body: JSON.stringify({ key: `file:${key}`, type: 'context', content: String(data) }),
      });
    },
    async get(key) {
      const mem = await api<{ memory: Array<{ key: string; content: string }> }>(`/v1/agents/${agentId}/memory`);
      const entry = mem.memory.find(m => m.key === `file:${key}`);
      return entry ? new ReadableStream({ start(c) { c.enqueue(new TextEncoder().encode(entry.content)); c.close(); } }) : null;
    },
    async delete(key) {
      await fetch(`${base}/v1/agents/${agentId}/memory/file:${encodeURIComponent(key)}`, {
        method: 'DELETE',
        headers,
      });
    },
    async list(prefix) {
      const mem = await api<{ memory: Array<{ key: string; content: string; updatedAt: string }> }>(`/v1/agents/${agentId}/memory`);
      return mem.memory
        .filter(m => m.key.startsWith('file:') && (!prefix || m.key.startsWith(`file:${prefix}`)))
        .map(m => ({ key: m.key.replace('file:', ''), size: m.content.length, uploaded: m.updatedAt }));
    },
  };

  const subscription: SubscriptionClient = {
    async checkout() { return api('/v1/billing/checkout', { method: 'POST' }); },
    async portal() { return api('/v1/billing/portal', { method: 'POST' }); },
    async status() { return api('/v1/billing/status'); },
  };
  ...
  return {
    ...
    async chat(message: string) {
      const res = await api<{ message: { content: string } }>(`/v1/agents/${agentId}/chat`, {
        method: 'POST',
    ...
    async messages(limit = 50) {
      const res = await api<{ messages: unknown[] }>(`/v1/agents/${agentId}/messages?limit=${limit}`);
      return res.messages;
    },
    memory: {
      async list() {
        const res = await api<{ memory: unknown[] }>(`/v1/agents/${agentId}/memory`);
        return res.memory;
      },
      ...
      async delete(key) {
        await fetch(`${base}/v1/agents/${agentId}/memory/${encodeURIComponent(key)}`, {
          method: 'DELETE',
          headers,
        });
      },
    ...
    tasks: {
      async list() {
        const res = await api<{ tasks: unknown[] }>(`/v1/agents/${agentId}/tasks`);
        return res.tasks;
      },
