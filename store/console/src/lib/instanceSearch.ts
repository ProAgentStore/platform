...
//
// Substring, case-insensitive, over the fields that NAME an instance:
//
...
//   slug       the stable handle. Not on the card, but it is what appears in URLs and in every
//              MCP tool call, so it is frequently the string a user actually has in mind.
//   agentName  the agent's own name, which the server sends only for a RENAMED instance (#815).
//              It is the card's subtitle, so it is on screen — and without it, renaming "Repo
//              Coder" to "FAS platform" made that instance unfindable by what it is.
//
...
export function instanceMatches(inst: Instance, query: string): boolean {
	...
	if (!q) return true;
	return [inst.name, inst.slug, inst.agentName].some((field) => (field || "").toLowerCase().includes(q));
}
