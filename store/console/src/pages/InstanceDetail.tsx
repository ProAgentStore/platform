			...
			<div className="flex-1 overflow-hidden flex flex-col min-h-0">
				{tab === "chat" && (
					<div className="flex flex-col flex-1 min-h-0">
						...
						<div className="flex flex-wrap gap-1 px-2 py-1 shrink-0 items-center">
							<button type="button" onClick={voice.toggleMic} title="Talk: tap once, speak, and it sends your message when you pause (one at a time)" className={`flex items-center gap-1 px-2 py-1.5 text-sm border rounded-lg transition-colors ${voice.micOn ? "border-accent bg-accent-soft text-accent" : "border-line text-muted hover:border-accent hover:text-accent"}`}><Mic size={13} /><span className="text-[0.7rem] font-medium">Talk</span></button>
							<button type="button" onClick={voice.toggleSpeak} title="Speak replies: read every agent reply aloud (doesn't listen)" className={`flex items-center gap-1 px-2 py-1.5 text-sm border rounded-lg transition-colors ${voice.speakOn ? "border-accent bg-accent-soft text-accent" : "border-line text-muted hover:border-accent hover:text-accent"}`}><Volume2 size={13} /><span className="text-[0.7rem] font-medium">Speak</span></button>
							<button type="button" onClick={voice.toggleConvo} title="Hands-free: continuous conversation — you talk, it replies aloud, then listens again" className={`flex items-center gap-1 px-2 py-1.5 text-sm border rounded-lg transition-colors ${voice.convoOn ? "border-green bg-green/15 text-green" : "border-line text-muted hover:border-accent hover:text-accent"}`}><AudioLines size={13} /><span className="text-[0.7rem] font-medium">Hands-free</span></button>
							{voice.convoOn && <button type="button" onClick={voice.toggleMute} title={voice.muted ? "Unmute the mic" : "Mute the mic (stay in hands-free)"} className={`flex items-center gap-1 px-2 py-1.5 text-sm border rounded-lg transition-colors ${voice.muted ? "border-red bg-red/15 text-red" : "border-line text-muted hover:border-accent hover:text-accent"}`}><MicOff size={13} /><span className="text-[0.7rem] font-medium">{voice.muted ? "Muted" : "Mute"}</span></button>}
							{loopOn ? (
