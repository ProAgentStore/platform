...
(function () {
  ...
  authEl.style.cssText = 'display:inline-flex;align-items:center;gap:0.5rem;margin-left:0.25rem';
  ...
  if (!token) {
    ...
    link.style.cssText = 'color:#a3a3a3;text-decoration:none;font-size:0.88rem;font-weight:600';
  ...
  fetch(`${API}/v1/auth/me`, { headers: { Authorization: `Bearer ${token}` } })
    ...
    .then(user => {
      ...
      wrapper.style.cssText = 'display:inline-flex;align-items:center;gap:0.4rem;text-decoration:none';

      const avatar = document.createElement('img');
      avatar.src = user.avatar || '';
      avatar.alt = user.login;
      avatar.style.cssText = 'width:26px;height:26px;border-radius:50%;border:2px solid #262626';
