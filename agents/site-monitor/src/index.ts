...
function diffSummary(oldText: string, newText: string): string {
  ...
  for (const l of newLines) if (!oldSet.has(l) && l.trim()) added.push(l.trim());
  for (const l of oldLines) if (!newSet.has(l) && l.trim()) removed.push(l.trim());

  const parts: string[] = [];
  if (removed.length) parts.push(`Removed: "${removed.slice(0, 2).join(' | ').slice(0, 120)}"`);
  if (added.length) parts.push(`Added: "${added.slice(0, 2).join(' | ').slice(0, 120)}"`);
...
export class SiteStateDO extends DurableObject<Env> {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === '/state' && request.method === 'GET') {
      ...
      if (!state) return new Response(JSON.stringify(null), { headers: { 'Content-Type': 'application/json' } });
    ...
    if (url.pathname === '/check' && request.method === 'POST') {
      ...
      try {
        ...
        return new Response(JSON.stringify({ changed: false, error: String(err), consecutive_errors: errors }), {
      ...
      const summary = changed ? diffSummary(prevText ?? '', normalized.slice(0, 4000)) : '';

      await this.ctx.storage.put<SiteStateData>('state', {
        url: siteUrl,
        hash: newHash,
        last_checked: new Date().toISOString(),
        last_changed: changed ? new Date().toISOString() : (prevState?.last_changed ?? null),
        consecutive_errors: 0,
      });
      await this.ctx.storage.put<string>('normalized', normalized.slice(0, 4000));
...
async function runCron(env: Env): Promise<{ checked: number; changed: number; errors: number }> {
  ...
  ).bind('webhook_url').first<{ value: string }>();
  ...
  for (const site of sites.results) {
    ...
    if (result.error) {
      errors++;
      console.error(`[site-monitor] Error checking ${site.url}: ${result.error}`);
      continue;
    }
    ...
    if (result.changed) {
      ...
      await env.DB.prepare(
        `INSERT INTO change_history(id, site_id, old_hash, new_hash, summary, content_len)
         VALUES (?, ?, ?, ?, ?, ?)`,
      ).bind(historyId, site.id, result.oldHash, result.newHash, result.summary, result.contentLen).run();
      ...
      if (webhookUrl) {
        ...
        try {
          ...
          console.error(`[site-monitor] Webhook delivery failed for ${site.url}: ${err}`);
        }
      }
    }
  }
...
app.post('/sites', async (c) => {
  const body = await c.req.json<{ url: string; label?: string; webhook_url?: string }>();
  ...
  // Deduplicate by URL
  const existing = await c.env.DB.prepare('SELECT id FROM sites WHERE url = ?').bind(body.url).first<{ id: string }>();
  if (existing) return c.json({ error: 'url already monitored', id: existing.id }, 409);

  const id = nanoid();
  await c.env.DB.prepare(
    'INSERT INTO sites(id, url, label, webhook_url) VALUES (?, ?, ?, ?)',
  ).bind(id, body.url, body.label ?? '', body.webhook_url ?? null).run();

  return c.json({ id, url: body.url, label: body.label ?? '', webhook_url: body.webhook_url ?? null }, 201);
});

// DELETE /sites/:id — remove a URL
app.delete('/sites/:id', async (c) => {
  const { id } = c.req.param();
  const site = await c.env.DB.prepare('SELECT id FROM sites WHERE id = ?').bind(id).first<{ id: string }>();
...
app.get('/sites', async (c) => {
  ...
  const withState = await Promise.all(results.map(async (site) => {
...
app.get('/sites/:id/history', async (c) => {
  ...
  ).bind(id, limit, offset).all<ChangeHistory>();
...
app.get('/config', async (c) => {
  const row = await c.env.DB.prepare('SELECT value FROM config WHERE key = ?').bind('webhook_url').first<{ value: string }>();
...
app.put('/config', async (c) => {
  ...
  if (typeof body.webhook_url !== 'string') return c.json({ error: 'webhook_url is required' }, 400);
  if (body.webhook_url && !body.webhook_url.startsWith('https://')) {
    return c.json({ error: 'webhook_url must start with https://' }, 400);
  }
  await c.env.DB.prepare('INSERT OR REPLACE INTO config(key, value) VALUES (?, ?)').bind('webhook_url', body.webhook_url).run();
  ...
  async scheduled(_event: ScheduledEvent, env: Env, ctx: ExecutionContext) {
    ...
    ctx.waitUntil(
      runCron(env).then(({ checked, changed, errors }) => {
        console.log(`[site-monitor] done — checked=${checked} changed=${changed} errors=${errors}`);
      }),
    );
  },
