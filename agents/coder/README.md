...
**Your AI coding agent for any GitHub repo.** Subscribe, point it at a repository you own, and it runs a coding engine (Claude Code, Gemini CLI, Codex, Grok, or a local command) on **your own machine** in that repo — you drive it from any device, or hand it an objective and let it work autonomously, with live human takeover when it gets stuck.
...
- **Hands (your machine):** run `pags up`. The runner prepares the repo you imported and launches the selected coding engine in that directory — the same local runner used for the browser runtime, with a second `coding` runtime. Claude runs as a persistent structured session; non-Claude engines run one-shot turns and stream their captured output back to the console.
...
| Piece | Where |
|---|---|
| Local coding runtime (hands) | `packages/browser-runner/src/coding/` |
...
Not ported from AgentCoder: GCP cloud-VM provisioning (local runner only), standalone Stripe (folds into the OFO $9 model), the Discord interface.
