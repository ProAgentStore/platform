...
const CONTENT_TYPE_INSTRUCTIONS: Record<ContentType, string> = {
  ...
  social: 'Write a social media post suitable for LinkedIn and Twitter/X. Keep it punchy, include relevant hashtags at the end.',
  newsletter: 'Write a newsletter section with a subject line, a warm greeting, the main content, and a call-to-action sign-off.',
};
...
async function runPipeline(env: Env) {
  ...
  const topicRes = await stub.fetch(new Request('http://do/next-topic', { method: 'POST' }));
  ...
  const aiResult = await env.AI.run(
    ...
    {
      messages: [
        { role: 'system', content: 'You are an expert content writer. Follow all instructions precisely.' },
  ...
  await stub.fetch(new Request('http://do/index-content', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ id, key: r2Key }),
  }));

  console.log(`[content-pipeline] Generated ${config.contentType} for topic "${topic.text}" → ${r2Key}`);
}
...
export class PipelineDO extends DurableObject<Env> {
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === '/topics' && method === 'GET') {
      ...
      const topics = [...entries.values()].sort(
    ...
    if (url.pathname === '/config' && method === 'PUT') {
      ...
      const current = (await this.ctx.storage.get<Config>('config')) ?? { ...DEFAULT_CONFIG };

      const VALID_CONTENT_TYPES: ContentType[] = ['blog', 'social', 'newsletter'];
      const VALID_TONES: Tone[] = ['professional', 'casual', 'witty', 'inspirational', 'educational'];
      const VALID_LENGTHS: Length[] = ['short', 'medium', 'long'];

      if (patch.contentType !== undefined && !VALID_CONTENT_TYPES.includes(patch.contentType)) {
        return json({ error: `contentType must be one of: ${VALID_CONTENT_TYPES.join(', ')}` }, 400);
      }
      if (patch.tone !== undefined && !VALID_TONES.includes(patch.tone)) {
        return json({ error: `tone must be one of: ${VALID_TONES.join(', ')}` }, 400);
      }
      if (patch.length !== undefined && !VALID_LENGTHS.includes(patch.length)) {
        return json({ error: `length must be one of: ${VALID_LENGTHS.join(', ')}` }, 400);
      }
