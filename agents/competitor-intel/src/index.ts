...
async function summarizeChange(
  ...
  const result = (await ai.run('@cf/meta/llama-3.3-70b-instruct-fp8-fast' as Parameters<Ai['run']>[0], {
    messages: [
      {
        role: 'system',
        content:
...
async function generateReportSummary(
  ...
  return result.response?.trim() ?? 'See individual competitor summaries below.';
}
...
app.delete('/competitors/:id', async (c) => {
  ...
  const doId = c.env.INTEL.idFromName('main');
  const stub = c.env.INTEL.get(doId);
  const res = await stub.fetch(new Request(`http://do/competitors/${c.req.param('id')}`, {
    method: 'DELETE',
  }));
  return new Response(res.body, { status: res.status, headers: res.headers });
});
...
export default {
  ...
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext) {
    ...
    const competitorsRes = await stub.fetch(new Request('http://do/competitors'));
    const { competitors } = await competitorsRes.json<{ competitors: Competitor[] }>();
    ...
    for (const comp of competitors) {
      ...
      if (changed && prevState) {
        ...
        changeSummary = await summarizeChange(env.AI, comp.url, comp.label, prevText, text);
      }

      // Persist new state + snapshot
      await stub.fetch(new Request(`http://do/state/${comp.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          hash,
          lastFetchedAt: new Date().toISOString(),
          lastChangedAt: changed ? new Date().toISOString() : (prevState?.lastChangedAt ?? null),
        } satisfies CompetitorState),
      }));
      await stub.fetch(new Request(`http://do/snapshot/${comp.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'text/plain' },
        body: text,
      }));

      items.push({ competitorId: comp.id, url: comp.url, label: comp.label, changed, changeSummary });
    }
    ...
    await stub.fetch(new Request('http://do/reports', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export class CompetitorIntelDO extends DurableObject<Env> {
  async fetch(request: Request): Promise<Response> {
    ...
    if (path === '/competitors' && method === 'GET') {
      const list = (await this.ctx.storage.get<Competitor[]>('competitors')) ?? [];
      return json({ competitors: list });
    }

    // POST /competitors
    if (path === '/competitors' && method === 'POST') {
      const { url: compUrl, label } = await request.json<{ url: string; label?: string }>();
      const list = (await this.ctx.storage.get<Competitor[]>('competitors')) ?? [];
    ...
    if (path === '/reports' && method === 'GET') {
      ...
      const summaries = [...entries.values()].map(({ items: _items, ...r }) => r);
