...
const DEFAULT_CRITERIA: ScoringCriteria = {
  hotSignals: [
    'urgent', 'asap', 'immediately', 'ready to buy', 'ready to start',
    'enterprise', 'budget approved', 'decision maker', 'ceo', 'cto', 'vp',
    'looking for a solution', 'evaluating', 'demo',
  ],
  coldSignals: [
    'just curious', 'student', 'research', 'no budget', 'someday',
    'maybe', 'not sure', 'free', 'cheap',
  ],
  hotThreshold: 70,
  warmThreshold: 40,
};

const SCORING_MODEL = '@cf/meta/llama-3.3-70b-instruct-fp8-fast' as Parameters<Ai['run']>[0];
...
function doFetch(stub: DurableObjectStub, path: string, init?: RequestInit): Promise<Response> {
...
app.post('/webhook/ingest', async (c) => {
  ...
  const { score, scoreValue, notes } = await scoreLead(c.env.AI, {
    name, email,
    company: company || '',
    message: message || '',
    extra,
  ...
  const lead: Lead = {
    ...
    extra: Object.fromEntries(
      Object.entries(extra).filter(([, v]) => typeof v === 'string') as [string, string][],
    ),
    score,
    scoreValue,
    notes,
...
app.put('/leads/:id/status', async (c) => {
  ...
  const validStatuses: LeadStatus[] = ['new', 'contacted', 'qualified', 'disqualified', 'converted'];
...
export class LeadStoreDO extends DurableObject<Env> {
  ...
  private async computeStats(): Promise<Stats> {
    ...
    const byStatus: Record<LeadStatus, number> = {
      new: 0, contacted: 0, qualified: 0, disqualified: 0, converted: 0,
    };
...
async function scoreLead(
  ai: Ai,
  lead: { name: string; email: string; company: string; message: string; extra: Record<string, string | undefined> },
...
{"score": <0-100 integer>, "classification": "hot" | "warm" | "cold", "notes": "<2-3 sentence reasoning>"}`;
  ...
  try {
    const result = await ai.run(SCORING_MODEL, {
      messages: [
        { role: 'system', content: 'You are a lead scoring assistant. Output only valid JSON.' },
    ...
    if (jsonMatch) {
      ...
      if (parsed.classification === 'hot' || parsed.classification === 'warm' || parsed.classification === 'cold') {
        score = parsed.classification;
      } else {
        // Derive from numeric score if model classification is missing/wrong
        score = scoreValue >= criteria.hotThreshold
          ? 'hot'
    ...
    console.error('AI scoring failed:', err instanceof Error ? err.message : String(err));
    // Fall back to heuristic scoring
    const text = `${lead.company} ${lead.message}`.toLowerCase();
    const hotHits = criteria.hotSignals.filter((s) => text.includes(s)).length;
    const coldHits = criteria.coldSignals.filter((s) => text.includes(s)).length;
...
async function notifyHotLead(webhookUrl: string, lead: Lead): Promise<void> {
  try {
    ...
    console.error('Hot lead notification failed:', err instanceof Error ? err.message : String(err));
  }
}
