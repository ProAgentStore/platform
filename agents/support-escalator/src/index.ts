...
export class TicketStoreDO extends DurableObject {
	...
	async fetch(request: Request): Promise<Response> {
		...
		if (request.method === "GET" && path === "/do/open-summary") {
			const open = [...this.tickets.values()].filter(
	...
	return [
		{
			...
			answer:
				"You can cancel your subscription at any time from your account settings under Billing > Cancel Plan. Your access continues until the end of the current billing period.",
		},
		{
			id: "kb-2",
			topic: "billing",
		...
		},
		{
			...
			answer:
				'Click "Forgot Password" on the login page and enter your email. You will receive a reset link within a few minutes. Check your spam folder if it does not arrive.',
		},
		{
			id: "kb-4",
			topic: "account",
			...
			answer:
				"Go to Account Settings > Profile and update your email. You will need to verify the new address before the change takes effect.",
		},
		{
			id: "kb-5",
			topic: "technical",
			...
			answer:
				"Try clearing your browser cache and cookies, then reload. If the issue persists, try a different browser or disable browser extensions. You can also check our status page for any ongoing incidents.",
		},
		{
			id: "kb-6",
			topic: "technical",
			...
			answer:
				"Please note the exact error message and the steps that led to it. Common fixes include refreshing the page, clearing cache, or logging out and back in. If the issue continues, our team will investigate.",
		},
	];
}
...
async function classifyTicket(
	...
	classification: "auto" | "escalate";
	confidence: string;
	reason: string;
}> {
	const kbSummary = kb.map((e) => `- ${e.topic}: "${e.question}"`).join("\n");
...
Keep it concise and actionable. Plain text only.`;
	...
	return (
		result.response?.trim() ?? `${openTickets.length} tickets need attention.`
	);
}
...
app.post("/webhook/ticket", async (c) => {
	...
	if (!subject || !ticketBody || !customerEmail) {
		return c.json(
			{ error: "subject, body, and customerEmail are required" },
			400,
		);
	}
	...
	// Classify with AI
	const { classification, reason } = await classifyTicket(
		c.env,
		subject,
		ticketBody,
		kb,
...
app.get("/tickets", async (c) => {
	...
	const res = await doFetch(
		do_,
		`/do/tickets${status ? `?status=${status}` : ""}`,
	);
...
app.put("/tickets/:id/resolve", async (c) => {
	const do_ = getDoStub(c.env);
	const res = await doFetch(do_, `/do/tickets/${c.req.param("id")}/resolve`, {
		method: "PUT",
	});
	if (!res.ok) return c.json({ error: "Ticket not found" }, 404);
	return c.json(await res.json<Ticket>());
